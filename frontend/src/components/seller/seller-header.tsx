...
import { SELLER_ROUTES } from "./seller-sidebar";
...
export function SellerHeader() {
  ...
  return (
    <>
      <header className="h-20 bg-white border-b border-black/5 flex items-center justify-between px-6 md:px-10 sticky top-0 z-40 lg:hidden">

        {/* Left: Mobile Menu & Brand */}
        <div className="flex items-center gap-4 flex-1">
          <button
            onClick={() => setIsMobileMenuOpen(true)}
            className="p-2 -ml-2 text-[rgba(10,10,10,0.6)] hover:bg-black/5 rounded-xl transition-colors"
          >
            ...
            >
              <div className="h-20 flex items-center justify-between px-6 border-b border-black/5 flex-shrink-0">
                <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2">
                  <span className="text-[20px]">🎓</span>
                  <span className="text-[18px] font-bold tracking-tight">GradMart <span className="text-[9px] bg-[#0a0a0a] text-white px-2 py-0.5 rounded-full ml-1 font-bold">SELLER</span></span>
                </Link>
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
