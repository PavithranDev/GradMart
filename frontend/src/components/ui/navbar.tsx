  ...
  return (
    <>
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[90%] md:w-[600px]">
        ...
        >
          <Link 
            href={status === "authenticated" ? (session?.user?.role === "SELLER" ? "/seller" : session?.user?.role === "ADMIN" ? "/admin" : "/dashboard") : "/"} 
            className="flex items-center gap-2 text-white px-4 hover:opacity-80 transition-opacity flex-shrink-0"
          >
          ...
          <div className="flex items-center gap-2 pr-2">
            <Link href="/search" className="hidden md:flex w-8 h-8 items-center justify-center rounded-full text-white hover:bg-white/10 transition-colors">
