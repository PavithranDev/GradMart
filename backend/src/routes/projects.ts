...
router.get('/', async (req, res) => {
  try {
    ...
    if (sort === 'Price: Low to High') orderByClause = { price: 'asc' };
    else if (sort === 'Price: High to Low') orderByClause = { price: 'desc' };
    else if (sort === 'Most Popular') orderByClause = { sales: 'desc' }; // wait, where sales=0... Most popular might be by rating
    else if (sort === 'Highest Rated') orderByClause = { rating: 'desc' };
